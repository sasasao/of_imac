...
void testApp::draw(){
    ...
    float r = 100;
    float ellipseX = r*cos(t);
    float ellipseY = r*sin(t);
    
    ofVec2f evenPos = ofVec2f(ellipseX + ofGetWidth()/2 ,ellipseY + ofGetHeight()/2);
    ofVec2f pointNorm = ofVec2f(evenPos) / ofGetWindowSize();
    ofVec2f pointVel = ofVec2f(evenPos - pMouse) / ofGetWindowSize();
    addToFluid(pointNorm, pointVel, true, true);
    
    for (int i=0; i<ofGetHeight(); i+=100) {
        ofSetColor(0);
        ofDrawEllipse(100, i, 10, 10);
        ofVec2f eventPos = ofVec2f(100,i);
        ofVec2f mouseNorm = ofVec2f(eventPos) / ofGetWindowSize();
        ofVec2f mouseVel = ofVec2f(10,10) / ofGetWindowSize();

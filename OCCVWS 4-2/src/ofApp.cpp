...
void ofApp::setup(){
    ...
    font.load("Bauhaus-Bold.otf",24,true,true,true);
    
    //dataフォルダに配置したcsvファイルの名前
    string filePath = "003_ST_new.csv";
    string filePath1 = "002_ST_new.csv";
...
void ofApp::draw(){
    ...
    if(color_foc == 0 && color_sle == 0){
        color_foc = 255;
        color_sle = 255;
        alpha = 20;
    }
    if(color_foc1 == 0 && color_sle1 == 0){
        color_foc1 = 255;
        color_sle1 = 255;
        alpha = 20;
    }
    ...
    for (int i = 0; i < 1; i++){
        ...
        float y = 55 + ((ofGetHeight()-80-150) /15)*i;
        font.drawStringAsShapes(ofToString(i+9), x, y);
...
void ofApp::loadCsvToMemes(string filePath){
    ...
    for (ofBuffer::Line it = buffer.getLines().begin(), end = buffer.getLines().end(); it != end; ++it) {
        ...
        if (data.size()>=1) {
             ...
             */
            meme.eyeMoveRout = ofToFloat(data[1]);
            meme.cadence = ofToFloat(data[0]);
            meme.blink = ofToFloat(data[3]);
            meme.sleepy = ofToFloat(data[4]);
            meme.focus = ofToFloat(data[2]);
...
void ofApp::loadCsvToMemes1(string filePath1){
    ...
    for (ofBuffer::Line it = buffer.getLines().begin(), end = buffer.getLines().end(); it != end; ++it) {
        ...
        if (data.size()>=1) {
             ...
             */
            meme1.eyeMoveRout1 = ofToFloat(data[1]);
            meme1.cadence1 = ofToFloat(data[0]);
            meme1.blink1 = ofToFloat(data[3]);
            meme1.sleepy1 = ofToFloat(data[4]);
            meme1.focus1 = ofToFloat(data[2]);
